...
class Graph{
	...
	public function add_star($nodes_for_star){
		...
		for($i = 1;$i < count($nodes_for_star); $i++){
			...
			$arr=[$middle];
			array_push($arr,$nodes_for_star[$i]);
	...
	public function add_cycle($nodes_for_cycle){
...
// kisat
	...
	public function min_mximal_matching(){
		$arr=[];
		array_push($arr,$this->edges[0]);
		foreach ($this->edges as $key) {
			for($i=0;$i<count($arr);$i++){
			if (!in_array($key[0],$arr[$i])&&!in_array($key[1], $arr[$i])) {
				print "<pre>";
		print_r(count($arr));
				array_push($arr, $key);
			}
